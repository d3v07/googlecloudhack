...
init = wait_for(rid)
if "error" in init:
    print("INIT FAILED:", init); proc.kill(); sys.exit(1)
...
required = ["explain","mongodb-logs","create-index","drop-index","collection-indexes",
           "aggregate","find","count","collection-schema","atlas-get-performance-advisor"]
...
results["find"] = summarize(call_tool("find", {
    "database": "sample_supplies", "collection": "sales", "limit": 1}))
results["count"] = summarize(call_tool("count", {
    "database": "sample_supplies", "collection": "sales"}))
results["aggregate"] = summarize(call_tool("aggregate", {
    "database": "sample_supplies", "collection": "sales",
    "pipeline": [{"$count": "n"}]}))
results["collection-schema"] = summarize(call_tool("collection-schema", {
    "database": "sample_supplies", "collection": "sales", "sampleSize": 20}))
results["collection-indexes"] = summarize(call_tool("collection-indexes", {
    "database": "sample_supplies", "collection": "sales"}))
results["explain"] = summarize(call_tool("explain", {
    "database": "sample_supplies", "collection": "sales",
    "method": [{"name": "find", "arguments": {"filter": {"storeLocation": "Denver"}}}],
    "verbosity": "executionStats"}))
results["mongodb-logs"] = summarize(call_tool("mongodb-logs", {
    "type": "global", "limit": 5}))

# Write tools on scratch namespace (insert a seed doc first so collection exists)
call_tool("insert-many", {"database": SCRATCH_DB, "collection": SCRATCH_COLL,
                          "documents": [{"x": 1}, {"x": 2}]})
results["create-index"] = summarize(call_tool("create-index", {
    "database": SCRATCH_DB, "collection": SCRATCH_COLL, "name": "smoke_x",
    "definition": [{"type": "classic", "keys": {"x": 1}}]}))
# drop-index has a confirmation gate; call may return a confirmation request
drop_resp = call_tool("drop-index", {
    "database": SCRATCH_DB, "collection": SCRATCH_COLL,
    "indexName": "smoke_x", "type": "classic"})
results["drop-index"] = summarize(drop_resp)

# Atlas Performance Advisor (expected deferred on M0 / no service account)
pa = call_tool("atlas-get-performance-advisor", {
    "projectId": os.environ.get("ATLAS_PROJECT", ""),
    "clusterName": "target",
    "operations": ["suggestedIndexes"]})
results["atlas-get-performance-advisor"] = summarize(pa)

for k in ["explain","find","count","aggregate","collection-schema","collection-indexes",
          "mongodb-logs","create-index","drop-index","atlas-get-performance-advisor"]:
