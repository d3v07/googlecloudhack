...
def req(path, body=None, bearer=None, xapi=False, timeout=200):
    ...
    return json.load(
        u.urlopen(
            u.Request(BASE + path, data=data, headers=h, method="POST" if data else "GET"),
            timeout=timeout,
        )
    )
...
print(
    "Dev      ran phone_seniors  -> sev",
    r1["captured"]["signal"]["severity"],
    "docs",
    r1["captured"]["metrics"]["docs_examined"],
)
r2 = req("/workload/query", {"preset": "seattle_recent"}, bearer=aakash)
print(
    "Aakash   ran seattle_recent -> sev",
    r2["captured"]["signal"]["severity"],
    "docs",
    r2["captured"]["metrics"]["docs_examined"],
)
...
print(
    f"memory: status={mem['status']} configured={mem['configured']} "
...
json.dump(
    {
        "run_id": rid,
        "phone_captured_id": cid,
        "memory_status": mem["status"],
        "memory_sources": srcs,
    },
    open("/Users/dev/Documents/GCRAH/demo/video/prestage.json", "w"),
    indent=2,
)
